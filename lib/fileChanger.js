...
function appendProvideGetter(body, moduleName, options) {
  ...
  let moduleNameString = JSON.stringify(moduleName);
  return body += `
;(function() {
  ...
  function($controllerProvider, $compileProvider, $filterProvider) {
    ...
    if (window.Proxy) {
      (${_proxifyScopeFromCompile})(window, $compileProvider);
    }
  }])${ie8Content};
})();
`;
  ...
  function $injularCompileProviderGet() {
    ...
    function $injularCompile(elements) {
      ...
      function injularLinkScope(scope) {
        var proxyScope = new window.Proxy(scope, { get: get });
