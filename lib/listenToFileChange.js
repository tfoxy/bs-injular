...
function listenToFileChange(config, bs) {
  ...
  function matchFile(data) {
    ...
    if (match(url, config.templates)) {
      ...
      let templateOptions = { supportIE8: !config.screwIE8 };
      bs.io.sockets.emit(TEMPLATE_CHANGED_EVENT, {
        template: fileChanger.wrapTemplate(content, url, templateOptions),
        templateUrl: url,
      ...
      if (recipes.length) {
        ...
        let content = fs.readFileSync(data.path, 'utf8');
        if (directiveMatched) {
          content = fileChanger.wrapDirectiveFile(content);
        }
