...
describe('template changed listener (scope)', function() {
  ...
  var ngApps = ['app'];
  if (window.Proxy) {
    ngApps.push('injularApp');
  }

  angular.forEach(ngApps, function(ngApp) {

    describe('(with ' + ngApp + ')', function() {
      ...
      if (ngApp === 'injularApp') {
        it('should compile template only once when using Proxy', function() {
          var template = [
            '<!--bs-injular-start /app/foo.html-->',
            '<i ng-controller="Foo"></i>',
            '<!--bs-injular-end /app/foo.html-->'
          ].join('');
          var element = angular.element([
            '<div ng-app="injularApp">',
            template,
            '</div>'
          ].join(''));
          rootElement.append(element);
          angular.bootstrap(element, ['injularApp', registerControllers]);
          element.injector().get('$templateCache').put('/app/foo.html', template);

          expect(element.find('i').scope()).to.have.property('foo', 1);
          ...
          var prevFooValue = element.find('i').scope().foo;
          ...
          expect(element.find('i').scope()).to.have.property('foo', prevFooValue + 1);
